...
| ~~`Recurrence` type~~ | #29 — **DROPPED** | — | — | **Not shipped** (issue #29 closed *not planned*). RFC 5545 RRULE + `rrule` dep was overkill for the only real use case (spaced-repetition date advancing). Superseded by the `Duration`/`MultiDuration` cycling below — same workflow, **zero dep**. Kept here as the record of why. |
...
| `MultiDuration` type | (folds #29's workflow into #30) | YAML list of DURATIONs | — | The list counterpart of `Duration` (ordered interval sequence stored in the note). `MultiDurationEditorModal` add/remove/reorder, each item via `DurationInputModal`. **Drives the spaced-repetition mechanic**: a Date/DateTime field's option `nextIntervalField` names a `Duration`/`MultiDuration` field; the date editor's **"Set next date"** button computes `current + head interval`, writes it, and for a MultiDuration **rotates the list head→tail** (wraps after the last) — one immediate `processFrontMatter` write (`nextDateProvider` in `fieldActions`). Not Formula/Lookup: manual, single-note, no auto-recompute. |
